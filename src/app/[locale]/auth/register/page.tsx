...
export default function RegisterPage() {
  ...
  // Check for Google OAuth return with phone linking
  const linkPhoneMutation = trpc.auth.linkPhone.useMutation();

  useEffect(() => {
    const linkPhone = searchParams.get("linkPhone");
    if (linkPhone === "1") {
      const verifiedPhone = sessionStorage.getItem("carsouk_verified_phone");
      const redirect = searchParams.get("redirect") || `/${locale}`;
      if (verifiedPhone) {
        linkPhoneMutation.mutate({ phone: verifiedPhone }, {
          onSuccess: async () => {
            sessionStorage.removeItem("carsouk_verified_phone");
            await updateSession({ phone: verifiedPhone });
            window.location.href = redirect;
          },
          onError: () => {
            sessionStorage.removeItem("carsouk_verified_phone");
            window.location.href = `/${locale}/auth/verify-phone?callbackUrl=${encodeURIComponent(redirect)}`;
          },
        });
      } else {
        // No phone in sessionStorage — middleware will redirect to verify-phone
        window.location.href = redirect;
      }
    }
  ...
  const handleGoogleSignIn = () => {
    sessionStorage.setItem("carsouk_verified_phone", phone);
    const params = new URLSearchParams(window.location.search);
    const callbackUrl = params.get("callbackUrl") || `/${locale}`;
    signIn("google", { callbackUrl: `/${locale}/auth/register?linkPhone=1&redirect=${encodeURIComponent(callbackUrl)}` });
  };
          ...
          {step === 2 && (
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              ...
              <Button type="submit" className="w-full" disabled={verifyOtpMutation.isPending}>
                {verifyOtpMutation.isPending && <Loader2 className="me-2 h-4 w-4 animate-spin" />}
                {t("verify")}
              </Button>
          ...
          {step === 3 && (
            <>
              <div className="flex items-center gap-2 rounded-md bg-green-50 px-3 py-2 text-sm text-green-700 dark:bg-green-900/20 dark:text-green-400">
                ...
                <span className="ms-auto font-mono text-xs" dir="ltr">{phone}</span>
              </div>
              ...
              <form onSubmit={handleRegister} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="name" type="text" placeholder={t("namePlaceholder")} value={name} onChange={(e) => setName(e.target.value)} className="ps-10" required autoComplete="name" autoFocus />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="email" type="email" placeholder={t("emailPlaceholder")} value={email} onChange={(e) => setEmail(e.target.value)} className="ps-10" required autoComplete="email" />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input id="password" type={showPassword ? "text" : "password"} placeholder={t("passwordPlaceholder")} value={password} onChange={(e) => setPassword(e.target.value)} className="ps-10 pe-10" required minLength={8} autoComplete="new-password" />
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Lock className="absolute start-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                    <Input id="confirmPassword" type={showPassword ? "text" : "password"} placeholder={t("confirmPasswordPlaceholder")} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className="ps-10" required minLength={8} autoComplete="new-password" />
                  </div>
                </div>
                ...
                <p className="text-center text-xs text-muted-foreground">
                  {t("terms")}{" "}
                  <Link href="/legal/terms" className="text-primary hover:underline">{t("termsLink")}</Link>{" "}
                  &{" "}
                  <Link href="/legal/privacy" className="text-primary hover:underline">{t("privacyLink")}</Link>
                </p>
              </form>
